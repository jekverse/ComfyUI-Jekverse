"""
# Sidebar ComfyUI-Jekverse Extension for ComfyUI
...
def get_filename_from_url(url, platform, civitai_token=None):
    ...
    try:
        ...
        # Use a streaming GET request because many APIs (like CivitAI) block HEAD requests (403 or 405)
        ...
        # If Content-Disposition isn't present but it redirected to a URL with a filename
        if resp.history and urlparse(resp.url).path:
            redirect_filename = os.path.basename(unquote(urlparse(resp.url).path))
            if redirect_filename and '.' in redirect_filename:
                resp.close()
                return redirect_filename.split('?')[0]
                
        resp.close()
    except Exception as e:
        print(f"[Downloader] Error in auto-detecting filename: {e}")
...
def prepare_url(url, platform, civitai_token=None):
...
def download_with_aria2(item_id, url, directory, custom_filename, platform, civitai_token=None):
    """Download using aria2c with parallel connections."""
    global current_process, cancel_requested
    
    # Check aria2 installed, try to auto-install if missing.
    try:
        subprocess.run(['aria2c', '--version'], capture_output=True, check=True)
    except Exception:
        update_item(item_id, message="Installing aria2c package...")
        add_log("⚙️ aria2c is missing. Attempting to install it automatically...", "info")
        try:
            subprocess.run(['apt-get', 'update'], capture_output=True, check=True)
            subprocess.run(['apt-get', 'install', '-y', 'aria2'], capture_output=True, check=True)
            add_log("✅ aria2c successfully installed!", "success")
        except Exception as e:
            update_item(item_id, status="error", message="Failed to install aria2c.")
            add_log(f"❌ Failed to auto-install aria2. Please install manually: apt install aria2. Error: {e}", "error")
            return False
    
    # Determine filename
    filename = custom_filename if custom_filename else get_filename_from_url(url, platform, civitai_token)
    ...
    prepared_url = prepare_url(url, platform, civitai_token)
    ...
    # Passing Referer to CivitAI R2 cloudflare redirects causes 403 Forbidden with aria2, so we don't pass it here.
...
def download_with_hf(item_id, url, directory, custom_filename, hf_token=None):
...
def process_queue():
    ...
    while not cancel_requested:
        ...
        hf_token = next_item.get("hf_token", "")
        civitai_token = next_item.get("civitai_token", "")
...
@PromptServer.instance.routes.post("/downloader/detect-filename")
async def api_detect_filename(request):
    try:
        ...
        filename = get_filename_from_url(url, platform, civitai_token if civitai_token else None)
